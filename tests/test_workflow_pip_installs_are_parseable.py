...
# The same suite runs inside the built Lambda image, whose /var/task carries
# the application tree but not `.github/` -- nothing there is a workflow, so
# there is nothing here to assert. Declared with the `repo_tree` marker that
# `docker-image-tests` deselects, NOT a skip on the directory's absence: a skip
# would also swallow a real discovery regression, which
# test_the_workflow_dir_actually_has_workflows below exists to catch
